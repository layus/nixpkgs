# This file defines generic procedures to wrap executables.
# This is done by hiding away the executable, and replacing it with a small bash script that

# A header for wrappers, so we can tell them from other scripts
__wrapperHeader="# This wrapper was generated by makeWrapper"
...
assertExecutable() {
    local file="$1"
    # Ensure that $1 is an executable file, or a symlink thereto"
    [[ -f "${file}" && -x "${file}" ]] || \
        die "Cannot wrap ${file} because it is not an executable file"
    # Sanity check: refuse to wrap wrappers
    if grep -Fxq "${__wrapperHeader}" "${file}"; then
        echo "Not wrapping ${file} because it is already a wrapper"
        return 1
    fi
}
...
makeWrapper() {
    ...
    assertExecutable "${original}" || return 0

    mkdir -p "$(dirname "$wrapper")"

    echo "#! $SHELL -e" > "$wrapper"
    echo "$__wrapperHeader" >> "$wrapper"
...
wrapProgram() {
    ...
    assertExecutable "${prog}" || return 0
