...
preConfigurePhases+=' sourceGNUstepConfig';
sourceGNUstepConfig () {
    . $GNUSTEP_MAKEFILES/GNUstep.sh
}

preFixupPhases+=" wrapGNUstepEnv"
wrapGNUstepEnv () {
    ...
    for i in $out/bin/* $out/sbin/*; do
        echo "wrapping $(basename $i) with GNUstep config"
        wrapProgram "$i" \
            --set GNUSTEP_CONFIG_FILE "$out/share/.GNUstep.conf"
    done
}
